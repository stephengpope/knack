...
· Tailwind v4 · shadcn (Base UI, **not Radix**) · AI SDK 6 via **Vercel AI Gateway**
· Better Auth · Drizzle · Neon Postgres.

### The agent request (`app/api/agent/route.ts`)
Heart of the app. Per POST: auth-gate → resolve model → ensure chat row →
build sandbox tools → run `ToolLoopAgent` → stream UI messages → persist on finish.
...
- Tools: `runBash`, `readFile`, `writeFile`, `listFiles` (sandbox) + `list_tokens`,
  `get_token` (per-user secrets vault). All sandbox ops go through one box per chat.
...
`account`/`verification`/`rate_limit`) + app tables (`chat`, `message`,
`api_key` shared BYOK, `app_settings` singleton, `custom_endpoint`, `user_secret`
per-user vault). `user_settings` is **deprecated** — kept only so migrations stay
additive; superseded by `app_settings`. Schema changes: edit `schema.ts` →
`db:generate` → `db:migrate`.

### Routes
- `(auth)/` — login, forgot/reset password, accept-invite.
- `(app)/` — protected shell: `page.tsx` (new chat), `chat/[chatId]`, `chats`,
  `settings` (per-user secrets), `administration` (admin: users, AI models, secrets).
...
- **Base UI, not Radix.** shadcn components here are Base UI. Triggers are buttons
  with **no `asChild`** — use `render` or move props onto the trigger.
