  ...
  return (
    <>
      ...
      <div className="mb-4 mt-7 flex items-center gap-3">
        ...
        <div className="ml-auto flex h-9 w-55 items-center gap-2 rounded-md border border-input bg-muted px-3 text-ink-faint focus-within:border-primary">
      ...
      <div className="overflow-hidden rounded-lg border border-border">
        <div className="grid grid-cols-[1fr_148px_44px] gap-3 border-b border-border bg-card px-4 py-2.5 text-xs font-bold uppercase tracking-label text-ink-faint">
          <div>Member</div>
          <div>Role</div>
          <div />
        </div>
  ...
  return (
    ...
    >
      <DialogTrigger asChild>
        <Button className="knack-gradient knack-glow ml-auto h-10 shrink-0 rounded-lg px-4 font-bold text-white">
          <UserPlus className="size-4" /> Invite members
        </Button>
      </DialogTrigger>
      <DialogContent
        showCloseButton={false}
        className="gap-0 overflow-hidden rounded-xl p-0 sm:max-w-130"
      >
        <InviteDialogBody
          onInvited={onInvited}
    ...
    <>
      ...
      <div className="px-6 pb-1.5">
        ...
        <div
          ...
          className="flex flex-wrap items-center gap-1.5 rounded-lg border border-input bg-muted p-2 focus-within:border-primary"
        >
          {emails.map((email, i) => (
            <span
              key={email}
              className="flex items-center gap-1.5 rounded-md border border-border bg-card py-1 pl-2 pr-1 text-sm font-semibold"
            >
              <span
              ...
              <button
                ...
                className="flex size-5 items-center justify-center rounded-sm text-ink-faint transition-colors hover:bg-accent hover:text-foreground"
              >
          ...
          <input
            ...
            className="min-w-35 flex-1 bg-transparent px-1 py-1 text-sm outline-none placeholder:text-ink-faint"
            autoFocus
          />
        </div>
        ...
        <FieldLabel className="mt-5">Role for these invites</FieldLabel>
      ...
      <div className="mt-3 flex items-center gap-3 border-t border-border bg-muted/40 px-6 py-4">
        <span className="ml-auto" />
        <Button variant="outline" onClick={onClose} className="h-10 rounded-lg">
          Cancel
        </Button>
        <Button
          onClick={send}
          disabled={busy || count === 0}
          className="knack-gradient knack-glow h-10 rounded-lg px-5 font-bold text-white"
        >
          {busy ? (
            <Spinner />
          ) : (
...
function DialogHeaderRow({ onClose }: { onClose: () => void }) {
  return (
    <div className="flex items-start gap-3 px-6 pb-4 pt-6">
      <span className="flex size-11 shrink-0 items-center justify-center rounded-lg bg-sidebar-accent text-accent-text">
        <UserPlus className="size-5" />
      </span>
      <div className="min-w-0 flex-1">
        <DialogTitle className="text-lg font-extrabold tracking-headline">
          Invite members
        </DialogTitle>
        <DialogDescription className="mt-0.5 text-sm text-ink-soft">
      ...
      <button
        onClick={onClose}
        className="flex size-8 items-center justify-center rounded-md text-ink-faint transition-colors hover:bg-accent hover:text-foreground"
      >
        <X className="size-4" />
      </button>
    </div>
...
function ResultsPanel({
  ...
  return (
    <>
      <div className="flex items-start gap-3 px-6 pb-4 pt-6">
        <span className="flex size-11 shrink-0 items-center justify-center rounded-lg bg-success-soft text-success">
      ...
      <div className="flex max-h-[50vh] flex-col gap-2.5 overflow-y-auto px-6 pb-2">
        {results.map((r) => (
          <div
            key={r.email}
          ...
          >
            <div className="flex items-center gap-2">
              <span className="truncate text-sm font-bold">{r.email}</span>
              {r.error ? (
                <span className="ml-auto shrink-0 rounded-full bg-muted px-2 py-0.5 text-xs font-bold text-ink-soft">
                  {r.error}
                </span>
              ) : r.emailed ? (
                <span className="ml-auto flex shrink-0 items-center gap-1 rounded-full bg-success-soft px-2 py-0.5 text-xs font-bold text-success">
      ...
      <div className="mt-3 flex justify-end border-t border-border bg-muted/40 px-6 py-4">
        <Button
          onClick={onClose}
          className="knack-gradient h-10 rounded-lg px-5 font-bold text-white"
        >
          Done
        </Button>
      </div>
...
function FieldLabel({
  ...
  return (
    <div
      className={cn(
        "mb-2 text-xs font-bold uppercase tracking-label text-ink-faint",
        className,
      )}
    >
      {children}
...
function RoleRadio({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex items-start gap-3 rounded-lg border-[1.5px] p-3 text-left transition-colors",
        selected
...
function CopyLink({ link, className }: { link: string; className?: string }) {
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-2 rounded-md border border-input bg-muted py-1.5 pl-3.5 pr-1.5",
        className,
      )}
    >
      ...
      <button
        ...
        className="flex size-8 shrink-0 items-center justify-center rounded-sm text-ink-soft transition-colors hover:bg-accent hover:text-foreground"
      >
        {copied ? (
    ...
    <div className="grid grid-cols-[1fr_148px_44px] items-center gap-3 border-b border-border px-4 py-3 transition-colors last:border-0 hover:bg-accent/40">
      ...
      {isSelf ? (
        ...
        <button
          ...
          disabled={busy}
          title="Remove user"
          className="flex size-8 items-center justify-center rounded-sm text-ink-faint transition-colors hover:bg-muted hover:text-foreground disabled:opacity-50"
        >
...
function RolePillDropdown({
  ...
  if (disabled) {
    return (
      <span className="inline-flex w-fit items-center gap-1.5 rounded-md border border-input bg-muted px-2.5 py-1.5 text-xs font-bold text-ink-soft">
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="inline-flex w-fit items-center gap-1.5 rounded-md border border-input bg-card px-2.5 py-1.5 text-xs font-bold transition-colors hover:bg-accent">
        <Icon className="size-3.5" />
        {label}
...
function RolesLegend() {
  ...
  return (
    <div className="mt-7 rounded-lg border border-border bg-card p-5">
      <div className="mb-3 text-sm font-bold">What roles can do</div>
      <div className="flex flex-col gap-2.5">
        {roles.map((r) => (
          <div key={r.name} className="flex items-start gap-3">
            <span className="w-16 shrink-0 text-xs font-bold text-accent-text">
              {r.name}
            </span>
